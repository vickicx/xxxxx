...
@interface ZRGZViewController ()

@end

@implementation ZRGZViewController
...
//    zrgzImageView.image = [UIImage imageNamed:@"zhuanrangguize"];
    ...
    [agreeButton setTitleColor:[UIColor groupTableViewBackgroundColor] forState:UIControlStateNormal];
    agreeButton.titleLabel.font = [UIFont systemFontOfSize:16 weight:6];
    [agreeButton addTarget:self action:@selector(agreeButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    
//    NSLog(@"转让规则model ====== %@", self.model);
}

- (void)agreeButtonClicked:(UIButton *)button
{
    SQZRViewController *sqzr = [[SQZRViewController alloc] init];
    
    sqzr.sid = self.sid;
    
    sqzr.xiangmuBJ = self.xiangmuBJ;
    
//    sqzr.model = self.model;
    
    sqzr.tender_from = self.tender_from;
    
    sqzr.oid_tender_id = self.oid_tender_id;
    
    sqzr.pageTag = self.pageTag;
    ...
    [self.navigationController pushViewController:sqzr animated:YES];
}
